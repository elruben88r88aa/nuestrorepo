...
def artPath():
    theme = appearance()
    if theme in ['-', '']: return
    elif condVisibility('System.HasAddon(plugin.video.exodus)'):
        return os.path.join(xbmcaddon.Addon('plugin.video.exodus').getAddonInfo('path'), 'resources', 'media', theme)


def appearance():
    appearance = setting('appearance.1').lower() if condVisibility('System.HasAddon(plugin.video.exodus)') else setting('appearance.alt').lower()
    return appearance


def artwork():
    execute('RunPlugin(plugin://plugin.video.exodus)')
...
def metaFile():
    if condVisibility('System.HasAddon(plugin.video.exodus)'):
        return os.path.join(xbmcaddon.Addon('plugin.video.exodus').getAddonInfo('path'), 'resources', 'data', 'meta.db')
